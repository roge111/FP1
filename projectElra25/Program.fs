...
let rec loop_r (array: int list, i: int) : int =
    if array[array.Length - 1] >= i then
...
let fibonachi (n: int) : int64 =
    ...
    let rec result (n: int, array: int64 list, i: int) : int64 =
        if array[i] >= n then array[i] else result (n, array, i + 1)
